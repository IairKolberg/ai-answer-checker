...
class TestYAMLValidation(unittest.TestCase):
    ...
    def test_all_agent_yaml_files_are_valid(self):
        ...
        for yaml_file in yaml_files:
            try:
                ...
                # Special case: agent-level service stub files
                # These files contain only `agent_stubs` and are not individual test cases.
                try:
                    with open(yaml_file, 'r', encoding='utf-8') as f:
                        raw_data = yaml.safe_load(f)
                except Exception as e:
                    raw_data = None  # Let the analyzer handle syntax errors uniformly
                
                if isinstance(raw_data, dict) and 'agent_stubs' in raw_data and 'user_input' not in raw_data and 'expected_answer' not in raw_data:
                    stub_errors = self._validate_agent_stub_file(yaml_file)
                    if stub_errors:
                        invalid_files[yaml_file] = stub_errors
                    # Skip TestCase parsing for agent-level files
                    continue
    ...
    def _validate_agent_stub_file(self, yaml_file: Path) -> list:
        """Validate structure of an agent-level service stub YAML file.
        Ensures `agent_stubs` is a mapping of tool names to a list of request/response specs,
        and referenced response files exist under the agent's `stubs/` directory.
        ...
        agent_stubs = data.get('agent_stubs')
        if not isinstance(agent_stubs, dict):
            return ["`agent_stubs` must be a mapping of tool name -> list of request specs"]
        
        stubs_root = yaml_file.parent / 'stubs'
        for tool_name, requests in agent_stubs.items():
            if not isinstance(tool_name, str) or not tool_name:
                errors.append("Tool name in `agent_stubs` must be a non-empty string")
                continue
            if not isinstance(requests, list) or not requests:
                errors.append(f"`agent_stubs.{tool_name}` must be a non-empty list")
                continue
            for idx, req in enumerate(requests):
                if not isinstance(req, dict):
                    errors.append(f"`agent_stubs.{tool_name}[{idx}]` must be a mapping with 'request' and 'response_file'")
                    continue
                if 'request' not in req or 'response_file' not in req:
                    errors.append(f"`agent_stubs.{tool_name}[{idx}]` must contain 'request' and 'response_file'")
                    continue
                if not isinstance(req['request'], dict):
                    errors.append(f"`agent_stubs.{tool_name}[{idx}].request` must be a mapping")
                if not isinstance(req['response_file'], str) or not req['response_file']:
                    errors.append(f"`agent_stubs.{tool_name}[{idx}].response_file` must be a non-empty string path")
                else:
                    # Verify the response file exists relative to the agent's stubs/ directory
                    expected_path = stubs_root / req['response_file']
                    if not expected_path.exists():
                        errors.append(f"Response file not found: {expected_path}")
        return errors
