...
class TestConfigService:
    ...
    def discover_test_files(self, agent_name: str) -> List[Path]:
        ...
        # Filter out any hidden or backup files and agent-level services file(s)
        yaml_files = [
            f for f in yaml_files
            if not f.name.startswith('.') and f.name not in ('agent-services.yaml', 'agent-services.yml')
        ]
